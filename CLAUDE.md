...
**Phase 12 📋 KẾ HOẠCH (61–63, nén 3 ngày) — LLM UI catalog + bug fix batch (`docs/17-roadmap-phase-12.md`):**
...
| 63 | Tests + CI + Audit + Cổng P12 | Tests mới ~30 (catalog/test-conn/bug-fixes) · CI matrix sqlite+postgres xanh · target ≥490 tests · READ-ONLY audit · 4 nguyên tắc · degrade safe · đóng pha · BUILD_STATE/CLAUDE | ☐ |

**Ràng buộc lõi Phase 12:** KHÔNG thêm engine feature/tool/schema — chỉ biên dashboard/intake · LLM key không bao giờ rời server (chỉ bool `key_set`) · test connection = prompt tối giản, không chạy investigation · 4 nguyên tắc + READ-ONLY giữ · CI phải xanh trên cả sqlite + postgres · **GreenNode MaaS** (VNG Cloud AI Platform, base `https://maas-llm-aiplatform-hcm.api.vngcloud.vn/v1`, OpenAI-compat) chạy qua `OpenAICompatibleClient` hiện có — chỉ thêm catalog metadata, KHÔNG sửa client/factory.
...
**Phase 12** 📋 (Ngày 61–63, nén 3 ngày, `docs/17`, KẾ HOẠCH TIẾP THEO): LLM UI catalog + bug fix batch — `llm/catalog.py` PROVIDER_CATALOG (8 provider, **+GreenNode MaaS** VNG Cloud cho model self-hosted) + provider/model `<select>` + base_url auto-fill + option "Khác" · test connection endpoint + UI badge · fix key security (không lộ HTML) + key preserve · fix port hardcoded 8000 (BUG-01) · fix admin nav `or true` (BUG-02) · fix pricing prefix stale (BUG-05) · fix slack thiếu channels grid (BUG-06). Không thêm engine/tool/schema (MaaS qua OpenAICompatibleClient hiện có). ≥490 tests target.
