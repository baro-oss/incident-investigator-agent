...
**Phase 1–4 ✅ HOÀN TẤT (20/20 ngày). Phase 5 ✅ HOÀN TẤT (25/25 ngày). Phase 6 ✅ HOÀN TẤT (26–30). Phase 7 ✅ HOÀN TẤT (31–35 — 63/63 tests). Phase 8 ✅ HOÀN TẤT (36–45 — 173/173 tests). Phase 9 ✅ HOÀN TẤT (46–50 — 262/262 tests) — engine lõi thông minh hơn: E10 tool-sequencing · E11 service prior · E12 specificity gate. Phase 10 ✅ HOÀN TẤT (51–55 — 444/444 tests) — agent đọc mã nguồn qua external MCP (GitHub/GitLab): F1 code seam + READ-ONLY guard · F2 deploy↔code synergy + E10/E11/E12 · V1 eval harness + E13 prior decay · P2 distill tổng quát · OPS1 catalog editor · T3 coverage 55%. Phase 11 ✅ HOÀN TẤT (56–60) — Postgres Tier-2 + cloud container deploy: PG backend qua storage seam · dialect parity + đóng rò seam · Dockerfile prod + secrets fail-fast · SIGTERM drain + trace retention + bug B1/B2. Single-instance, deploy fresh. 461/461 tests. Phase 12 ✅ HOÀN TẤT (61–63, nén 3 ngày) — LLM UI catalog + bug fix batch: PROVIDER_CATALOG 8 provider (+GreenNode MaaS) · provider/model dropdown + base_url auto-fill · test connection endpoint + UI badge · key security (không lộ HTML) + key preserve · BUG-01..07 fix · 502/502 tests. Phase 13 ✅ HOÀN TẤT (64–69) — Hardening & Sharpening: H1/H2/H3 silent-death+dừng-sớm+PG-crash · M1-M12 parity+specificity+deps+code-diff+conn-leak · M5-M7 READ-ONLY guard+authz+SSE auth · L1/L4/L5/L8 guards+escape+HMAC-warn · 594/594 tests. Phase 14 📋 ĐÃ LÊN KẾ HOẠCH (Ngày 70–73) — Bug Fix & UX Batch: BUG-14-01 MCP URL per-project · BUG-14-02 service delete URL · BUG-14-03 mask inputs · UX-14-01/02/03 · ENG-14-01 service context. Bug registry: `docs/19-roadmap-phase-14.md`. Uncommitted: BUG-14-01 + UX batch đã code (chờ commit). TODO: BUG-14-02 + BUG-14-03.**

Plan 20 ngày gốc: `docs/10-roadmap-20-ngay.md`. Plan Phase 5: `docs/11-roadmap-phase-5.md`. Plan Phase 6: `docs/12-roadmap-phase-6.md`. Plan Phase 8 (ĐÃ XONG): `docs/13-roadmap-phase-8.md`. Plan Phase 9 (ĐÃ XONG): `docs/14-roadmap-phase-9.md`. Plan Phase 10 (ĐÃ XONG): `docs/15-roadmap-phase-10.md`. Plan Phase 11 (ĐÃ XONG): `docs/16-roadmap-phase-11.md`. Plan Phase 12 (ĐÃ XONG): `docs/17-roadmap-phase-12.md`. Plan Phase 13 (ĐÃ XONG): `docs/18-roadmap-phase-13.md`. **Plan Phase 14 (ĐÃ LÊN KẾ HOẠCH, ĐANG CODE): `docs/19-roadmap-phase-14.md`** — Bug Fix & UX Batch (Ngày 70–73): BUG-14-01 MCP URL scope · BUG-14-02 service delete URL · BUG-14-03 mask inputs · UX-14-01 service desc · UX-14-02 trigger combobox · UX-14-03 channels · ENG-14-01 service context. **Ràng buộc: KHÔNG đụng engine state ngoài `service_descriptions` (đã thêm); KHÔNG schema thêm ngoài `project_services.description` (đã có migration).**
...
**Phase 12 ✅ HOÀN TẤT (61–63, nén 3 ngày) — LLM UI catalog + bug fix batch (`docs/17-roadmap-phase-12.md`):**

| Ngày | Theme | Nội dung | Trạng thái |
|------|-------|----------|-----------|
| 61 | LLM catalog + provider/model dropdown | `src/agent/llm/catalog.py` PROVIDER_CATALOG (8 provider, +**GreenNode MaaS** VNG Cloud) · `GET /dashboard/llm-catalog` · provider `<select>` + model `<select>` + base_url auto-fill + option "Khác" cho self-hosted · thêm `together`+`greennode` vào SUPPORTED_PROVIDERS | ✅ |
| 62 | Test connection + key security + bug batch | `POST /dashboard/projects/{pid}/llm/test` + UI badge · fix key không lộ HTML (bool indicator) + key preserve khi save rỗng · BUG-01 port `8000→8080` · BUG-02 admin nav `or true` · BUG-04 version · BUG-05 pricing prefix · BUG-06 slack grid · BUG-07 silent exception | ✅ |
| 63 | Tests + CI + Audit + Cổng P12 | 41 tests mới (catalog/test-conn/bug-fixes) · 502/502 tổng · fix encrypt_secret patch + TestClient drain + NOT NULL bug · READ-ONLY audit · 4 nguyên tắc · đóng pha | ✅ |

**Cổng Phase 12 PASS:** PROVIDER_CATALOG 8 provider + GreenNode MaaS ✅ · provider/model dropdown + base_url auto-fill ✅ · test-conn endpoint + UI badge ✅ · key security (không lộ HTML) + key preserve ✅ · BUG-01..07 ✅ · 502/502 tests ✅ · READ-ONLY + 4 nguyên tắc ✅
...
**Phase 11** ✅ (Ngày 56–60, `docs/16`): Postgres Tier-2 + deploy lên **GreenNode AgentBase** — PG backend qua storage seam (psycopg shim + pool) · dialect parity + đóng rò seam `rbac.py` + CI matrix sqlite|postgres · **port 8080 + build amd64** + Dockerfile prod · deploy qua managed CR + `runtime.sh` (single-instance `min=max=1`) · SIGTERM drain + trace retention · bug fix B1/B2. Deploy fresh. 461/461 tests.
**Phase 12** ✅ (Ngày 61–63, nén 3 ngày, `docs/17`): LLM UI catalog + bug fix batch — `llm/catalog.py` PROVIDER_CATALOG (8 provider, **+GreenNode MaaS** VNG Cloud cho model self-hosted) + provider/model `<select>` + base_url auto-fill + option "Khác" · test connection endpoint + UI badge · fix key security (không lộ HTML, `llm_key_set` bool) + key preserve · BUG-01..07 fix · 41 tests mới · 502/502 tổng. Không sửa engine/tool/schema (MaaS qua OpenAICompatibleClient hiện có).
**Future:** MySQL backend (seam sẵn) · bidirectional integration / code action (phá READ-ONLY, cần duyệt) · horizontal scale seam (externalize queue/dedup/SSE + Redis, cho multi-replica) · real-LLM eval đầy đủ (chờ credit, harness từ P10) · LLM catalog tự cập nhật từ provider API.
